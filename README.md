...
This project is a Derivative Calculator that can convert your given equation into a LaTeX equation using a parser and Abstract Syntax Tree (AST), which then finds the derivative in LaTeX. It implements flask, to create a funcitonal site that lets you input your desired equation to find its derivative.

![Example of the site](pictures/flask_example.png)
...
* All elementary functions (except logarithms with unique bases) are supported these include:  
    * Square root. $\sqrt{}$ ✅
    * Trigonometric functions. Like $\tan$ ✅
    * Inverse Trig functions. Like $\arccos$ ✅
    * However when using Inverse Trig functions, $\sin^{-1}$ and such is not supported ❌
    * Hyperbolic Trig functions. Like $\sinh$ ✅
    * The two basic logarithms will work, $\ln$ and $\log$ ✅
    * Logarithms with unique bases such as $\log_e$ (log_e), $\log_{10}$ (log_10), $\log_2$ (log_2) is not supported ❌
